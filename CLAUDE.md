...
- **codecompanion.lua**: Primary AI interface
  ...
  - Keymaps: `<Leader>av/as/at` for different chat layouts, `<Leader>ag` opens a dedicated "AI workspace" tmux session (first invocation launches `claude agents` in a window named `agents[ ]`; subsequent invocations add a new window running plain `claude`, mouse scrolling enabled)
...
- `<Leader>ag`: Open the "AI workspace" tmux session — first invocation creates the session with `claude agents` in an `agents[ ]` window; subsequent invocations add a new window running plain `claude` (mouse scroll enabled)
...
- Agent mode (`<Leader>ag`) opens a dedicated "AI workspace" tmux session with mouse scrolling enabled. The first invocation creates the session and launches `claude agents` (Claude Code agents view) in a window named `agents[ ]`. Subsequent invocations add a new window named `<cwd>[ ]` running plain `claude`. Auto-returns to the previous session when the last window closes.
